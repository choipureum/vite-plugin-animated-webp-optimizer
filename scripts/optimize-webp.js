...
args.forEach(arg => {
  ...
  if (arg.startsWith('--quality=')) config.quality = parseInt(arg.split('=')[1]);
...
async function optimizeWebP(inputPath, outputPath) {
  try {
    ...
    if (config.skipIfSmaller > 0 && stats.size < config.skipIfSmaller) {
      console.log(`⏭️  Skipping small file: ${fileName} (${formatBytes(stats.size)})`);
      return { skipped: true, originalSize: stats.size };
    }
    
    // Skip if file is too large
    if (config.maxFileSize > 0 && stats.size > config.maxFileSize) {
      console.log(`⏭️  Skipping large file: ${fileName} (${formatBytes(stats.size)})`);
      return { skipped: true, originalSize: stats.size };
    }
    ...
    // Detect if animated
    const metadata = await sharp(inputPath, { animated: true, pages: -1 }).metadata();
    ...
    if (savings > 0) {
      console.log(`   📉 Size: ${formatBytes(stats.size)} → ${formatBytes(optimizedSize)} (${savingsPercent}% saved)`);
    }
...
async function processBatch(files, batchSize) {
  ...
  for (let i = 0; i < files.length; i += batchSize) {
    ...
    console.log(`📊 Progress: ${progress}/${files.length} (${Math.round((progress / files.length) * 100)}%)`);
  }
...
async function main() {
  ...
  console.log(`⚡ Starting optimization with ${config.concurrentImages} concurrent processes...`);
  console.log('');
